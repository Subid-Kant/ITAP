...
export default function HistoryView() {
  ...
  if (selectedTargetId) {
    return (
      ...
      <motion.div key="details" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} style={{ padding: 24 }}>
        <AnimatedButton
          onClick={handleBack}
        ...
        {targetDetails && !detailsLoading && (
          <div className="stagger">
            <HoverCard className="panel" style={{ marginBottom: 24 }} tiltFactor={1}>
              ...
              <div className="panel-body">
                <div style={{ color: 'var(--text-muted)', fontSize: 13 }}>
                  Scanned on: {new Date(targetDetails.target.created_at + 'Z').toLocaleString()}
                </div>
                ...
                {targetDetails.threats.length === 0 ? (
                  ...
                  <table className="threat-table">
                    ...
                    <StaggeredList component="tbody">
                      {targetDetails.threats.map((threat, i) => (
                        <StaggeredItem key={threat.id} component="tr">
                          ...
                          <td style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12 }}>
                            {threat.detected_at ? new Date(threat.detected_at + 'Z').toLocaleString() : '—'}
                          </td>
                        </StaggeredItem>
                      ))}
                    </StaggeredList>
            ...
            {targetDetails.scans.length > 0 && (
              <HoverCard className="panel" style={{ marginTop: 16 }}>
                <div className="panel-header">
                ...
                <div className="panel-body no-pad">
                  <table className="threat-table">
                    ...
                    <StaggeredList component="tbody">
                      {targetDetails.scans.map((scan, i) => (
                        <StaggeredItem key={scan.id} component="tr">
                          ...
                          <td style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12 }}>
                            {scan.started_at ? new Date(scan.started_at + 'Z').toLocaleString() : '—'}
                          </td>
                        </StaggeredItem>
                      ))}
                    </StaggeredList>
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {historyList.length === 0 ? (
        <HoverCard className="panel" style={{ textAlign: 'center', padding: 48 }}>
            ...
            <table className="threat-table">
              ...
              <StaggeredList component="tbody">
                {historyList.map((item, i) => (
                  <StaggeredItem key={item.target_id} component="tr">
                    <td>
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text-muted)', fontSize: 13 }}>
                        <Calendar size={13} />
                        {new Date(item.created_at + 'Z').toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                      </div>
                    </td>
                    <td>
